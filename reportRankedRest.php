	...
	function GetAPIData($method, $data) {
		...
		*/
		...
		$proxyServer = "[Proxy server IP or hostname]";  
		$proxyUser = "[Username for Proxy]"; 
		$proxyPassword = "[password for proxy]";
		$username = '[API username usually username:company]';
		$secret = '[API Password]';
		...
		*/
		...
		$rc->setOption(CURLOPT_PROXY, $proxyServer);
		$rc->setOption(CURLOPT_PROXYPORT, "80");
		$rc->setOption(CURLOPT_PROXYUSERPWD, $proxyUser.":".$proxyPassword);
	...
	while (!$done && !$error) {
		...
		if ($rc->getStatusCode()==200) {
			$response=$rc->getWebResponse();
			$json=json_decode($response);
			//If report is ready the data will be displayed - needs tidying up for export to text file, or import to a database
			if ($json->report) {
				$done=true;
